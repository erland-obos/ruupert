...
This document defines how this project may adapt, extend, or override the general Claude Code instructions provided in
the
...
The `.claude` directory provides the authoritative instruction set for Claude Code when assisting with development. This
file describes when and how project-specific rules may supplement or override those instructions.

**Note:** In this project, all `.claude` instructions and the overrides defined here are used exclusively to configure
and control Claude Code's behavior. Other AI agents or models are not supported by this instruction set.
...
Claude Code must treat `.claude/PROJECT-OVERRIDES.md` as an execution-time modifier of the `.claude` instruction set.
The
following rules
apply:
...
This file constrains both autonomous and user-directed Claude Code behavior. When a user requests an operation that
conflicts
